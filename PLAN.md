...
## Summary
将项目明确拆成三部分：软件1采集打包端、软件2前端+Node.js Express后台、PostgreSQL数据库服务。最终数据流为：软件1通过 HTTP POST 发送数据包到软件2后台，软件2后台校验并写入 PostgreSQL；软件2前端只访问软件2后台 API，从数据库读取实时值、拓扑状态、历史、告警和接口状态。
...
- 软件2新增 Node.js Express 后台：
  - 目录建议为 `软件2/backend`。
  - 使用 `express` + `pg` 连接 PostgreSQL。
  - 后台配置通过 `.env` 管理：`HOST`、`PORT`、`DATABASE_URL`、`INGEST_TOKEN`、`CORS_ORIGIN`。
  - 提供软件1入库接口：
...
- 软件1对接方式：
  - 软件1只需修改 `receiver.baseUrl` 为软件2后台地址，例如 `http://软件2后台IP:8000`。
  - `receiver.ingestPath` 保持 `/api/ingest/packets`。
  - token 暂时允许为空；若配置 `INGEST_TOKEN`，软件1同步配置 `receiver.token`，后台校验 `Authorization: Bearer <token>`。
...
- PostgreSQL 部署形态：
  - 数据库机器只开放 PostgreSQL 端口给软件2后台机器。
  - 软件1和软件2前端不直连数据库。
  - 默认数据库名建议：`pq_monitor`。
  - 默认用户建议：`pq_app`。
...
- 三机启动关系：
  - 数据库机器：启动 PostgreSQL，执行建表和种子脚本。
  - 软件2机器：启动 Express 后台，再托管/打开软件2前端。
  - 软件1机器：配置软件2后台地址，运行采集打包程序。
...
- 分机联调：
  - 数据库机器执行健康检查：软件2后台能连接 PostgreSQL。
  - 软件1机器访问 `http://软件2后台IP:端口/api/ingest/health` 成功。
...
- 软件2后台是唯一读写数据库的服务；软件1不直接连接 PostgreSQL。
